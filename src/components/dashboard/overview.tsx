  ...
  return <div className="overview-v2 space-y-6">
    ...
    <section>
      ...
      <div className="economics-summary-grid">
        ...
        <EconomicsMetric label="CPL" value={nullableCurrency(analytics.economics.cpl)} note={analytics.economics.coveredLeads!==coveredRows.length?String(analytics.economics.coveredLeads)+" supplier-reported leads · "+String(coveredRows.length)+" matched CRM records":String(analytics.economics.coveredLeads)+" covered leads"} onClick={()=>setDrilldown({title:"Covered leads behind CPL",subtitle:scopeLabel,leads:coveredRows.map(item=>item.lead),spendRows:spendRows.filter(item=>item.state==="known")})}/>
    ...
    <section>
      ...
      <Card className="p-5">
        ...
        {showSourceTable&&<div className="table-scroll mt-4"><table><thead><tr><th>Source</th><th>Spend</th><th>Leads</th><th>Qualified</th><th>Visits</th><th>Offers</th><th>Customers</th><th>Project value excl. VAT</th><th>Paid value</th><th>CPL</th><th>Cost / qualified</th><th>CAC</th><th>Cohort cash ROAS</th></tr></thead><tbody>
          {sortedSources.map(row=><tr key={row.source}><td className="font-semibold"><button type="button" className="client-link" onClick={()=>campaignFilter==="all"?openSource(row.source):setDrilldown({title:"Campaign records · "+campaignFilter,subtitle:scopeLabel,leads:analytics.rows.map(item=>item.lead),offers:selectedSentOffers,clients:selectedClients,spendRows})}>{row.source}</button></td><td>{row.costState==="missing"?"Cost missing":row.spend===null?"—":formatCurrency(row.spend)}</td><td>{row.deliveredLeads??row.leads}{row.deliveredLeads!==null&&<small className="block text-[var(--muted)]">{row.leads} matched CRM</small>}</td><td>{row.qualified}</td><td>{row.visits}</td><td>{row.offers}</td><td>{row.attributableClients}</td><td>{formatCurrency(row.projectValueExclVat)}</td><td>{formatCurrency(row.paidValue)}</td><td>{nullableCurrency(row.cpl)}</td><td>{nullableCurrency(row.costQualified)}</td><td>{nullableCurrency(row.cac)}</td><td>{nullableRatio(row.cohortCashRoas)}</td></tr>)}
        </tbody></table>{sortedSources.some(row=>row.deliveredLeads!==null)&&<p className="chart-footnote mt-3">Supplier-reported lead counts are shown when a verified source file exists. The matched CRM count stays visible and company-wide unique-lead totals continue to use CRM identities so overlapping suppliers are not double-counted.</p>}</div>}
      </Card>
      <Card className="mt-4 p-5">
