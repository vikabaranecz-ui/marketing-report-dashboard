...
export function LocationsPage({ data }: { data: CompanyDataset }) {
  return <div className="space-y-6">
    <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
      <Card className="p-5"><SectionHeader title="Qualified leads by municipality" description="CRM lead geography"/><ComparisonBars data={data.locations.map(l=>({name:l.municipality,value:l.qualified}))} accent={data.company.accent}/></Card>
      <Card className="p-5"><SectionHeader title="Revenue by municipality" description="Verified project value"/><ComparisonBars data={data.locations.map(l=>({name:l.municipality,value:l.revenue}))} accent="#191c1f"/></Card>
    </div>
    <Card className="p-5">
      <SectionHeader title="Location outcomes" description="No geographic ad cost is inferred. CPL and CAC require real location-level spend and are intentionally omitted."/>
      <div className="table-scroll"><table><thead><tr><th>Municipality</th><th>Leads</th><th>Qualified</th><th>Verified clients</th><th>Revenue</th><th>Avg. project</th></tr></thead><tbody>{data.locations.map(row=><tr key={row.municipality}><td className="font-semibold">{row.municipality}</td><td>{row.leads}</td><td>{row.qualified}</td><td>{row.won}</td><td>{formatCurrency(row.revenue)}</td><td>{formatCurrency(safeDivide(row.revenue,row.won))}</td></tr>)}</tbody></table></div>
    </Card>
  </div>;
}
