  ...
  return <div className="space-y-6">
    ...
    <Card className="p-5">
      ...
      <div className="table-scroll"><table>
        ...
        <tbody>{paidSourceRows.map(row=><tr key={row.source}><td className="font-semibold">{row.source}</td><td>{row.spend===null?"Cost missing":formatCurrency(row.spend)}</td><td>{row.deliveredLeads===null&&row.leads===0&&row.customers>0?<StatusPill tone="warn">Lead count missing</StatusPill>:<>{formatNumber(row.deliveredLeads??row.leads)}{row.deliveredLeads!==null&&<small className="block text-[var(--muted)]">{formatNumber(row.leads)} CRM-attributed</small>}</>}</td><td>{row.qualified}</td><td>{row.visits}</td><td>{row.offers}</td><td>{row.attributableClients}{row.undatedClients>0&&<small className="block text-amber-700">+{row.undatedClients} month unverified</small>}</td><td>{formatCurrency(row.projectValueExclVat)}</td><td>{formatCurrency(row.paidValue)}</td><td>{formatCurrency(row.cpl)}</td><td>{formatCurrency(row.cac)}</td><td className="font-semibold">{row.cohortCashRoas===null?"—":`${formatNumber(row.cohortCashRoas)}×`}</td></tr>)}</tbody>
      </table></div>
    </Card>
