...
import { AcquisitionPage, CampaignsPage, IntegrationsPage, LocationsPage, ServicesPage, SettingsPage, WebsiteSeoPage } from "./detail-pages";
...
export function DashboardShell({ bootstrap, section }: { bootstrap: DashboardBootstrap; section: SectionKey }) {
  ...
  const Page = useMemo(() => ({ overview: OverviewPage, acquisition: AcquisitionPage, "leads-sales": LeadsSalesPage, services: ServicesPage, campaigns: CampaignsPage, "website-seo": WebsiteSeoPage, locations: LocationsPage, integrations: IntegrationsPage, settings: SettingsPage })[section], [section]);
  function exportCsv() {
    if (!data) return;
  ...
  return <div className="app-shell"><Sidebar/><main className="main-shell"><TopControls companies={bootstrap.companies} companyId={companyId} onCompanyChange={setCompanyId} onExport={exportCsv}/><div className="page-wrap"><header className="page-header"><div><div className="flex items-center gap-2"><p className="eyebrow">{meta.eyebrow}</p>{bootstrap.mode === "demo" && <span className="demo-badge">Demo data</span>}</div><h1>{meta.title}</h1><p>{data ? `${data.company.name} · ${data.periodLabel} · ${data.comparisonLabel}` : "No company available"}</p></div><div className="company-mark" style={{background:data?.company.accent}}>{data?.company.shortName ?? "—"}</div></header>{data ? section === "leads-sales" && bootstrap.mode === "demo" ? <EmptyState title="Live data required" body="Leads & Sales never shows synthetic commercial metrics. Connect Supabase and the CRM / ROBAWS sources to use this page."/> : <Page data={data}/> : <EmptyState title="No companies available" body="Ask an administrator to assign your account to a company."/>}</div></main></div>;
}
