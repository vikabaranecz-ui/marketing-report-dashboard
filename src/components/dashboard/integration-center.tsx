  ...
  return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{integrations.map((item) => {
    ...
    return <Card className="p-5" key={item.id}>
      ...
      <dl className="mt-4 space-y-2 text-sm">
        ...
        {displayed.provider === "meta" && displayed.metaPermissionStatus && <div className="flex items-center justify-between gap-4"><dt className="text-[var(--muted)]">Lead Ads readiness</dt><dd><StatusPill tone={displayed.metaPermissionStatus === "Ready" ? "good" : displayed.metaPermissionStatus === "Missing permissions" ? "bad" : "neutral"}>{displayed.metaPermissionStatus}</StatusPill></dd></div>}
      </dl>
      {displayed.provider === "meta" && displayed.metaMissingPermissions?.length ? <p className="mt-3 text-xs leading-5 text-[var(--muted)]">Missing: {displayed.metaMissingPermissions.join(", ")}</p> : null}
...
function providerErrorFallback(provider: Integration["provider"], action: Action) {
  ...
  if (provider === "meta") return `${prefix}: verify access to the selected Meta ad account.`;
