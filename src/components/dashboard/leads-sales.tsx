...
import { formatCurrency, formatNumber, safeDivide } from "@/lib/metrics/kpis";
import { buildSourcePerformance, manualClientSource, normalizeAcquisitionSource } from "@/lib/metrics/business-overview";
...
export function LeadsSalesPage({ data }: { data: CompanyDataset }) {
  ...
  const commercialWonClients=(data.commercialClients??[]).filter(client=>client.commercialStatus==="CLIENT_WON");
  const sourceKnownClients=source
    ? commercialWonClients.filter(client=>{
        const manual=manualClientSource(data,client);
        if(manual&&normalizeAcquisitionSource(manual)===source)return true;
        if(!client.matchedLeadId)return false;
        const lead=data.leads.find(item=>item.id===client.matchedLeadId);
        return Boolean(lead&&normalizeAcquisitionSource(lead.source)===source);
      })
    : commercialWonClients;
  const sourceClientExternalIds=new Set(sourceKnownClients.map(client=>client.externalId));
  const sourceProjectRows=(data.allCommercialProjects??[]).filter(project=>Boolean(project.externalClientId&&sourceClientExternalIds.has(project.externalClientId)));
  const sourceProjectValue=sourceProjectRows.reduce((sum,project)=>sum+Number(project.valueInclVat??0),0);
  const sourceInvoiced=sourceKnownClients.reduce((sum,client)=>sum+client.invoicedTotal,0);
  const sourcePaid=sourceKnownClients.reduce((sum,client)=>sum+client.paidTotal,0);
  ...
  return <div className="space-y-8">
    ...
    <section aria-labelledby="executive-snapshot"><SectionTitle id="executive-snapshot" eyebrow="01 · Executive snapshot" title="Money and movement" description={source ? `Filtered to ${source}` : "All real CRM leads and verified ROBAWS evidence"} /><div className="sales-kpi-grid">
      <Metric label={selectedSourceRow?.deliveredLeads!==null&&selectedSourceRow?.deliveredLeads!==undefined?"Delivered leads":"Leads"} value={formatNumber(selectedSourceRow?.deliveredLeads??filteredLeads.length)} note={selectedSourceRow?.deliveredLeads!==null&&selectedSourceRow?.deliveredLeads!==undefined?String(filteredLeads.length)+" CRM-attributed records":"CRM records"} onClick={()=>setRecords({title:"Leads",subtitle:data.periodLabel,leads:filteredLeads})}/><Metric label="ROBAWS matched" value={formatNumber(matchedLeads.length)} note="Email, phone or unique exact name" onClick={()=>setRecords({title:"ROBAWS-matched leads",subtitle:data.periodLabel,leads:matchedLeads})}/><Metric label="Offers sent" value={formatNumber(filteredOffers.length)} note="CRM-linked individual offers" onClick={()=>setRecords({title:"ROBAWS offers",subtitle:data.periodLabel,offers:filteredOffers})}/><Metric label="Open offer value" value={formatCurrency(sum(openOffers, "priceInclVat"), true)} note={`${openOffers.length} waiting · CRM-linked`} money onClick={()=>setRecords({title:"Open offers",subtitle:data.periodLabel,offers:openOffers})}/><Metric label="Project detail value" value={formatCurrency(sourceProjectValue, true)} note="Detailed ROBAWS project rows" money onClick={()=>setRecords({title:"Detailed ROBAWS projects",subtitle:source??"All sources",projects:sourceProjectRows})}/><Metric label="Source-known clients" value={formatNumber(sourceKnownClients.length)} note={source?"Includes manually verified source clients":"All CLIENT_WON records"} onClick={()=>setRecords({title:"Source-known commercial clients",subtitle:source??"All sources",initialKind:"clients",clients:sourceKnownClients})}/><Metric label="Invoiced" value={formatCurrency(sourceInvoiced, true)} note="ROBAWS client ledger" money onClick={()=>setRecords({title:"Clients behind invoiced value",subtitle:source??"All sources",initialKind:"clients",clients:sourceKnownClients.filter(item=>item.invoicedTotal>0)})}/><Metric label="Paid value" value={formatCurrency(sourcePaid, true)} note="ROBAWS paid_total · not payment-date cash" money accent onClick={()=>setRecords({title:"Clients behind paid value",subtitle:source??"All sources",initialKind:"clients",clients:sourceKnownClients.filter(item=>item.paidTotal>0)})}/>
    </div></section>
    ...
    <section aria-labelledby="source-performance"><SectionTitle id="source-performance" eyebrow="03 · Source performance" title="Which sources create money?" description="Full source-known client value is shown here. Dated cohort clients are shown separately because monthly cohort CAC/ROAS requires a trustworthy acquisition date." /><div className="analytics-table-wrap"><table className="analytics-table source-performance-table"><thead><tr><th>Source</th><th>Leads</th><th>ROBAWS matched</th><th>Offers</th><th>Open offer €</th><th>Rejected offer €</th><th>Source project value €</th><th>Source-known clients</th><th>Dated cohort clients</th><th>Lead → client</th><th>Invoiced €</th><th>Paid €</th><th>Source CAC</th><th>Cohort CAC</th><th>Source paid ROAS</th><th>Cohort paid ROAS</th></tr></thead><tbody>{sourceRows.map(row => <tr key={row.source} className={source === row.source ? "is-selected" : ""}><td><button className="source-link" onClick={() => setSource(current => current === row.source ? null : row.source)}>{row.source}<span>{source === row.source ? "Filtered" : "Filter"}</span></button></td><td>{row.deliveredLeads===null&&row.crmLeads===0&&row.clients>0?<StatusPill tone="warn">Lead count missing</StatusPill>:<>{formatNumber(row.leads)}{row.deliveredLeads!==null&&<small className="block text-[var(--muted)]">{formatNumber(row.crmLeads)} CRM-attributed</small>}</>}</td><td>{formatNumber(row.matched)}</td><td>{formatNumber(row.offers)}</td><td><button className="money-link" onClick={() => focusOpenOffers(row.source)}>{formatCurrency(row.openValue, true)}</button></td><td>{formatCurrency(row.rejectedValue, true)}</td><td className="font-semibold text-[var(--ink)]">{formatCurrency(row.acceptedValue, true)}</td><td>{formatNumber(row.clients)}</td><td>{formatNumber(row.datedClients)}{row.undatedClients>0&&<small className="block text-amber-700">+{row.undatedClients} month unverified</small>}</td><td>{rate(row.clients, row.leads)}</td><td>{formatCurrency(row.invoiced, true)}</td><td className="font-semibold text-[var(--ink)]">{formatCurrency(row.paid, true)}</td><td>{row.cac===null?"—":formatCurrency(row.cac,true)}</td><td>{row.cohortCac===null?"—":formatCurrency(row.cohortCac,true)}</td><td>{row.roas===null?"—":`${row.roas.toFixed(2)}×`}</td><td>{row.cohortRoas===null?"—":`${row.cohortRoas.toFixed(2)}×`}</td></tr>)}</tbody></table></div></section>
...
function buildSourceRows(data: CompanyDataset, offers: CommercialOffer[], projects: CommercialProject[], invoices: CommercialInvoice[]) {
  ...
  return sources.map((source): SourceRow => {
    ...
    const clients = row?.sourceKnownClients ?? sourceLeads.filter(lead => verifiedMethods.has(lead.robawsMatchMethod) && lead.commercialStatus === "CLIENT_WON").length;
    const datedClients = row?.attributableClients ?? sourceLeads.filter(lead => verifiedMethods.has(lead.robawsMatchMethod) && lead.commercialStatus === "CLIENT_WON" && !hasDateConflict(lead.attributionLevel)).length;
    return {
      source,
      ...
      acceptedValue: row?.sourceProjectValueInclVat ?? sum(sourceProjects, "valueInclVat"),
      clients,
      datedClients,
      undatedClients: row?.undatedClients ?? 0,
      invoiced: row?.sourceInvoicedValue ?? sourceInvoices.reduce((total, invoice) => total + Math.max(0, invoice.totalInclVat - invoice.creditedTotal), 0),
      paid: row?.sourcePaidValue ?? sum(sourceInvoices, "paidTotal"),
      cost: row?.spend ?? null,
      cac: row?.spend===null||row?.spend===undefined?null:safeDivide(row.spend,clients),
      cohortCac: row?.cac ?? null,
      roas: row?.spend?row.sourcePaidValue/row.spend:null,
      cohortRoas: row?.cohortCashRoas ?? null,
    };
