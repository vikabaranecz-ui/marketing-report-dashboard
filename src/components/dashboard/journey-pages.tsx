...
function Funnel({ data }: { data: CompanyDataset }) {
  ...
  return <Card className="overflow-hidden"><div className="overflow-x-auto"><div className="flex min-w-[1320px] divide-x divide-[var(--line)]"><div className="min-w-[170px] bg-[var(--ink)] p-4 text-white"><span className="text-xs uppercase text-white/60">Acquisition scope</span><strong className="mt-2 block text-xl">{formatNumber(a.cohort.knownAcquired)}</strong><small className="mt-1 block text-[10px] text-white/45">{formatNumber(s.uniquePeople)} CRM-tracked · {formatNumber(a.cohort.supplierOnly)} supplier-only</small></div>{stages.map(([label,value])=><div key={label} className="min-w-[130px] flex-1 bg-white p-4"><span className="text-xs uppercase text-[var(--muted)]">{label}</span><strong className="mt-2 block text-xl">{value}</strong></div>)}</div></div><div className="border-t border-[var(--line)] bg-amber-50 px-4 py-3 text-xs text-amber-900"><strong>{s.qualifiedNoOffer} qualified people currently have no linked ROBAWS offer.</strong> Qualified means sales-relevant or progressed; it does not mean an offer already exists.</div></Card>;
}
