...
import type { CampaignMetric, ChannelMetric, Company, CompanyDataset, Lead, LocationMetric, ServiceMetric, TrendPoint } from "./types";
...
type RawIntegration = { id:string; provider:string; status:"connected"|"not_connected"|"error"; last_successful_sync:string|null; last_attempted_sync:string|null; sync_logs:{records_imported:number}[]|null };
...
async function loadLiveDataset(supabase: Awaited<ReturnType<typeof createSupabaseServerClient>>, company: Company): Promise<CompanyDataset> {
  ...
  const [metricsRes, leadsRes, quotesRes, projectsRes, revenueRes, servicesRes, campaignsRes, websiteRes, seoRes, integrationsRes] = await Promise.all([
    ...
    supabase.from("reporting_integration_connections").select("id,provider,status,last_successful_sync,last_attempted_sync,sync_logs(records_imported)").eq("company_id",company.id),
  ]);
...
function providerName(provider:string){return({meta:"Meta Ads",google_ads:"Google Ads",ga4:"Google Analytics 4",search_console:"Google Search Console",google_business:"Google Business Profile",monday:"CRM / Monday",website_forms:"Website forms"} as Record<string,string>)[provider]??provider;}
